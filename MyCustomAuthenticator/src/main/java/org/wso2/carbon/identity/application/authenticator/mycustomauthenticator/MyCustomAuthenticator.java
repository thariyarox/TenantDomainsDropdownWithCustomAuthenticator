...
import org.wso2.carbon.identity.application.authenticator.mycustomauthenticator.internal.MyCustomAuthenticatorServiceComponent;
 ...
 */
public class MyCustomAuthenticator extends AbstractApplicationAuthenticator
        implements LocalApplicationAuthenticator {

    private static final long serialVersionUID = 4438354156955223654L;
    private TenantManager tenantManager;
    private static volatile boolean isTenantManagerInitialized;

    private static Log log = LogFactory.getLog(MyCustomAuthenticator.class);

    private void initializeTenantManager() {

        if (!isTenantManagerInitialized) {
            synchronized (MyCustomAuthenticator.class) {
                if (!isTenantManagerInitialized) {
                    if (MyCustomAuthenticatorDataHolder.getInstance().getRealmService() != null) {
                        tenantManager = MyCustomAuthenticatorDataHolder.getInstance().getRealmService().getTenantManager();
                        isTenantManagerInitialized = true;
                    }
                }
        ...
        try {
            ...
            if (showAuthFailureReason != null && showAuthFailureReason.equals("true")) {
                ...
                if (errorCode != null && errorCode.equals(UserCoreConstants.ErrorCode.USER_IS_LOCKED)) {
                    ...
                    String activeTenantDomainList = getActiveTenantDomainsList();

                    if(activeTenantDomainList != null){
                        response.sendRedirect(response.encodeRedirectURL(loginPage + ("?" + queryParams))
                                + "&tenantList=" + activeTenantDomainList + "&authenticators=" + getName() + ":" + "LOCAL" + retryParam);
                    } else {
                        response.sendRedirect(response.encodeRedirectURL(loginPage + ("?" + queryParams))
                                + "&authenticators=" + getName() + ":" + "LOCAL" + retryParam);
                    }
    ...
    protected void processAuthenticationResponse(HttpServletRequest request,
        ...
        try {
            ...
            UserRealm userRealm = MyCustomAuthenticatorServiceComponent.getRealmService()
    ...
    public String getFriendlyName() {
        ...
        return MyCustomAuthenticatorConstants.AUTHENTICATOR_FRIENDLY_NAME;
    }

    @Override
    public String getName() {

        return MyCustomAuthenticatorConstants.AUTHENTICATOR_NAME;
    }


    /**
     * Method to retrieve the list of active tenant domains
     *
     * @return String of comma separated active tenant domains
     */
    private String getActiveTenantDomainsList() {
        ...
        if (tenantManager != null) {
            try {
                ...
                StringBuilder tenantListBuilder = new StringBuilder();
                for (Tenant tenant : tenants) {
                    if (tenant.isActive()) {
                        tenantListBuilder.append(tenant.getDomain() + ",");
                    }
                }
                if (tenantListBuilder.toString().length() > 0) {
                    tenantListBuilder.deleteCharAt(tenantListBuilder.toString().length() - 1);
                }

                tenantList = tenantListBuilder.toString();


            } catch (UserStoreException e) {
                log.error("Error while retrieving tenant list", e);
            }
        }
        return tenantList;
    }
